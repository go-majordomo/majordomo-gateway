...
// ListMetadataKeys returns every discovered metadata key across all API keys,
// highest approximate cardinality first so an operator can spot high-dimension keys
// (e.g. user_id) that should not be indexed.
func (r *MetadataKeyRepository) ListMetadataKeys(ctx context.Context) ([]*models.MetadataKey, error) {
	query := `
		SELECT ` + metadataKeyColumns + `, ak.name AS api_key_name
		FROM llm_requests_metadata_keys mk
		JOIN api_keys ak ON ak.id = mk.majordomo_api_key_id
		ORDER BY mk.approx_cardinality DESC, mk.key_name ASC`
