...
const apiKeyColumns = `id, key_hash, name, description, is_active, created_at, revoked_at, last_used_at, request_count, deprecated_model_behavior`
...
func (r *APIKeyRepository) CreateAPIKey(ctx context.Context, keyHash string, input *models.CreateAPIKeyInput) (*models.APIKey, error) {
	...
	if err := r.db.QueryRowxContext(ctx, query, keyHash, input.Name, input.Description).StructScan(&key); err != nil {
