...
func buildBedrockMetrics(input, output, cacheRead, cacheWrite int) *models.UsageMetrics {
	...
	return &models.UsageMetrics{
		...
		// Bedrock's Converse usage reports a single cacheWriteInputTokens with no
		// TTL breakdown; its cache writes use the 5-minute TTL (billed at 1.25x
		// input, like Anthropic's default), so attribute the whole amount to the
		// 5-minute bucket to keep 5m+1h == CacheCreationTokens.
